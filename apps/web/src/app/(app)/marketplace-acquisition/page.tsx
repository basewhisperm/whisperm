...
export default async function MarketplaceAcquisitionPage() {
  ...
  if (pipeline === null) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Run the marketplace pipeline seed before reviewing acquisition opportunities.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">Marketplace Acquisition</p>
        <h1 className="mt-2 text-2xl font-semibold text-foreground">Acquisition opportunities</h1>
        <p className="mt-1 text-sm text-muted-foreground">Review captured marketplace listings linked to CRM deals.</p>
      </div>

      {deals.length === 0 ? (
        <div className="rounded-2xl bg-background p-6 text-sm text-muted-foreground" style={{ border: "0.5px solid var(--color-border)" }}>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {deals.map((deal) => (
            ...
            >
              ...
              <p className="mt-3 text-sm font-semibold text-foreground">{formatValue(deal.value, deal.currency)}</p>
              <p className="mt-2 text-xs text-muted-foreground">Updated {deal.updatedAt.toLocaleString()}</p>
            </Link>
          ))}
        </div>
      )}
