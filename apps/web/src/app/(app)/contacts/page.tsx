...
type Stage = "Prospect" | "Qualified" | "Proposal" | "Engagement" | "Renewal" | string;
...
export default function ContactsPage() {
  ...
  const filtered = contacts
    ...
    .sort((a, b) => {
      ...
      switch (sortKey) {
        case "name":        av = getContactName(a); bv = getContactName(b); break;
      ...
      <div className="min-w-0 flex-1 space-y-4">
        ...
        <div className="overflow-hidden rounded-2xl" style={{ border: "0.5px solid hsl(var(--border))" }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">Loading contacts…</td></tr>
              ) : filtered.length === 0 ? (
                ...
                <tr key={contact.id} className="cursor-pointer hover:bg-secondary" style={{ borderTop: "0.5px solid hsl(var(--border))" }} onClick={() => setSelected(contact)}>
                  ...
                  <td className="px-4 py-3"><span className="font-medium" style={{ color: lastTouchColor(contact.lastTouchAt) }}>{lastTouchLabel(contact.lastTouchAt)}</span></td>
