...
const reveal=()=>{const nodes=Array.from(document.querySelectorAll('button,[role="button"],a.js-show-contact,a.qa-show-contact,a.cy-show-contact,button[class*="phone" i],button[class*="contact" i],[data-testid*="phone" i],[data-testid*="contact" i]'));const targets=nodes.filter((e)=>{const href=(e.getAttribute&&e.getAttribute("href")||"").toLowerCase();if(href.startsWith("tel:")||href.includes("wa.me")||href.includes("whatsapp"))return false;return /show\s+contact|show\s+phone|reveal\s+phone|phone|contact/i.test((e.textContent||"").trim())||(typeof e.matches==="function"&&e.matches('a.js-show-contact,a.qa-show-contact,a.cy-show-contact,button[class*="phone" i],button[class*="contact" i],[data-testid*="phone" i],[data-testid*="contact" i]'))}).slice(0,3);for(const c of targets){try{c.scrollIntoView({block:"center"});if(typeof c.focus==="function")c.focus();if(typeof c.click==="function")c.click();else ["pointerdown","mousedown","pointerup","mouseup","click"].forEach((t)=>c.dispatchEvent(new MouseEvent(t,{bubbles:true,cancelable:true,view:window})))}catch{}}};
const finish=(payload)=>{const json=JSON.stringify(payload);if(new TextEncoder().encode(json).length>MAX){alert("WhispeRM capture is too large. Narrow the visible marketplace results and try again.");return}if(payload.looksLikeGridPage&&!confirm("Capture the visible marketplace listings into WhispeRM discovery?"))return;const separator=INTAKE.includes("?")?"&":"?";window.open(INTAKE+separator+"payload="+encodeURIComponent(json),"_blank","noopener,noreferrer")};
const finishWhenRevealed=(remaining=40)=>{const payload=extractMarketplaceCapturePayload(document,location,(navigator&&navigator.userAgent)||"");if(payload.phone||remaining<=0){finish(payload);return}setTimeout(()=>finishWhenRevealed(remaining-1),250)};
reveal();
finishWhenRevealed();
