...
const createHarness = async (state, options = {}) => {
  ...
  writeFileSync(join(tempDir, 'provider-adapters.mjs'), [
    ...
    '  health: () => ([',
    '    { provider: "WHATSAPP", state: globalThis.__routeWhatsappProvider !== undefined ? "READY" : "UNCONFIGURED", configured: globalThis.__routeWhatsappProvider !== undefined, initialized: globalThis.__routeWhatsappProvider !== undefined, healthy: globalThis.__routeWhatsappProvider !== undefined, failureReason: null, lastInitializationAt: new Date(0).toISOString() },',
    '    { provider: "SMS", state: globalThis.__routeSmsProvider !== undefined ? "READY" : "UNCONFIGURED", configured: globalThis.__routeSmsProvider !== undefined, initialized: globalThis.__routeSmsProvider !== undefined, healthy: globalThis.__routeSmsProvider !== undefined, failureReason: null, lastInitializationAt: new Date(0).toISOString() },',
    '    { provider: "EMAIL", state: "UNCONFIGURED", configured: false, initialized: false, healthy: false, failureReason: null, lastInitializationAt: new Date(0).toISOString() },',
    '  ]),',
    '});',
    ...
    '});',
    // ST1-013J: mirrors the real invitation-provider-health contract (claim base URL first,
    // then channel provider health from the same registry execution uses) so this stub cannot
    // silently drift from the module it stands in for.
    'const CLAIM_URL_PLACEHOLDERS = ["changeme", "todo", "placeholder"];',
    'export function validateClaimBaseUrl(rawValue) {',
    '  const trimmed = rawValue?.trim();',
    '  if (!trimmed) return { ok: false, reason: "SELLER_INVITATION_BASE_URL is not set" };',
    '  if (CLAIM_URL_PLACEHOLDERS.some((token) => trimmed.toLowerCase().includes(token))) return { ok: false, reason: "SELLER_INVITATION_BASE_URL looks like a placeholder value" };',
    '  try { const url = new URL(trimmed); if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error("bad protocol"); } catch { return { ok: false, reason: "SELLER_INVITATION_BASE_URL is not an absolute URL" }; }',
    '  return { ok: true, url: trimmed };',
    '}',
    'export function buildClaimUrl(baseUrl, rawToken) { return `${baseUrl.replace(/\\/$/u, "")}/${rawToken}`; }',
    'export function checkInvitationProviderHealth({ channel, registry, env }) {',
    '  const e = env ?? process.env;',
    '  const claimValidation = validateClaimBaseUrl(e.SELLER_INVITATION_BASE_URL);',
    '  if (!claimValidation.ok) return { ok: false, code: "INVALID_CLAIM_BASE_URL", message: `Seller invitation claim link base URL is invalid: ${claimValidation.reason}.` };',
    '  const messagingProvider = channel === "WHATSAPP" ? "WHATSAPP" : channel === "SMS" ? "SMS" : "EMAIL";',
    '  const providerHealth = registry.health().find((entry) => entry.provider === messagingProvider);',
    '  if (!providerHealth || providerHealth.state !== "READY") {',
    '    return { ok: false, code: "MISSING_REQUIRED_ENV", message: `Invitation provider is missing required configuration for the ${messagingProvider.toLowerCase()} channel.`, diagnostics: { missingEnv: [] } };',
    '  }',
    '  return { ok: true, provider: messagingProvider.toLowerCase(), channel: messagingProvider.toLowerCase(), claimBaseUrl: claimValidation.url, diagnostics: { requiredEnvPresent: [] } };',
    '}',
    'export class ProviderDeliveryError extends Error {',
    '  constructor(input) { super(input.message); this.name = "ProviderDeliveryError"; this.provider = input.provider; this.status = input.status; this.safeCode = input.safeCode; this.safeCategory = input.safeCategory; this.requestId = input.requestId; this.retryable = input.retryable ?? false; }',
    '  toSafeDiagnostic() { return { provider: this.provider, status: this.status, code: this.safeCode, category: this.safeCategory, requestId: this.requestId, retryable: this.retryable }; }',
    '  toSafeMessage() { return `Provider rejected the invitation request (${this.provider}).`; }',
    '}',
    '',
  ].join('\n'));
