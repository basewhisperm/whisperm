...
test('marketplace acquisition page renders required board copy and stages', () => {
  ...
  assert.match(source, /Capture, invite, and convert marketplace sellers into Render sellers/u);
...
test('marketplace acquisition navigation is placed between deals and reports', () => {
  ...
  assert.equal(messages['marketplaceAcquisition.title'], 'Marketplace Acquisition');
...
test('deals API preserves default behavior and supports marketplace acquisition default key filtering', () => {
  ...
  assert.match(route, /pipelineDefaultKey === undefined\s*\? await pipelineRepo\.findByWorkspace/u);
  assert.match(route, /pipelineRepo\.findByDefaultKey\(workspaceId, pipelineDefaultKey\)/u);
  assert.match(route, /dealsRepo\.list\(workspaceId, \{ pipelineId: pipeline\.id \}\)/u);
});
