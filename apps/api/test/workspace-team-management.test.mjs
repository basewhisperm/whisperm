...
const createStore = ({ plan = "GROWTH", actorRole = "ADMIN" } = {}) => {
  ...
  return {
    ...
    sent: [],
    store: {
      async findWorkspace({ tenantId }) { return tenantId === "tenant-1" ? { id: "tenant-1", name: "Acme Workspace" } : null; },
      async findMember({ tenantId, userId }) { const member = members.get(userId); return member?.tenantId === tenantId ? member : null; },
      async findMemberByEmail({ tenantId, email }) { return [...members.values()].find((member) => member.tenantId === tenantId && member.email === email) ?? null; },
      async createMember(input) { const id = `user-${members.size + 1}`; const member = { id, ...input }; members.set(id, member); return member; },
      async updateMember(input) { const member = members.get(input.userId); assert.ok(member); const updated = { ...member, ...(input.role === undefined ? {} : { role: input.role }), ...(input.isActive === undefined ? {} : { isActive: input.isActive }) }; members.set(input.userId, updated); return updated; },
      async listMembers({ tenantId }) { return [...members.values()].filter((member) => member.tenantId === tenantId); },
      async findCurrentPlan() { return plan; },
      async createInvitation(input) { const invitation = { id: `invite-${invitations.size + 1}`, ...input }; invitations.set(invitation.id, invitation); return invitation; },
      async findInvitationByTokenHash({ tokenHash }) { return [...invitations.values()].find((invitation) => invitation.tokenHash === tokenHash) ?? null; },
      async listPendingInvitations({ tenantId, now: currentTime }) { return [...invitations.values()].filter((invitation) => invitation.tenantId === tenantId && invitation.acceptedAt == null && invitation.revokedAt == null && Date.parse(invitation.expiresAt) > currentTime.getTime()); },
      async markInvitationAccepted({ invitationId, acceptedAt }) { const invitation = invitations.get(invitationId); assert.ok(invitation); const accepted = { ...invitation, acceptedAt }; invitations.set(invitationId, accepted); return accepted; },
      async appendAudit(input) { audits.push(input); },
    },
    mailer: { async sendTeamInviteEmail(input) { this.sent.push(input); } },
  };
};
...
const createDependencies = (team) => ({
  ...
  apiKeyAuthenticator: { async authenticate(input) { return { tenantId: input.tenantId, apiKeyId: "key-1" }; } },
  ...
  workspaceTeamManagement: { store: team.store, mailer: { async sendTeamInviteEmail(input) { team.sent.push(input); } }, appBaseUrl: "https://app.example.com", now: () => now, tokenFactory: () => "fixed-token" },
});

const authHeaders = (overrides = {}) => ({ "x-tenant-id": "tenant-1", "x-user-id": "admin", "x-correlation-id": "corr-1", ...overrides });

const createInvite = (server, headers = authHeaders()) => server.inject({ method: "POST", url: "/workspaces/tenant-1/invitations", headers, payload: { email: "Person@Example.com", role: "MEMBER" } });
...
test("non-admin cannot create invitations or change roles", async () => {
  ...
  const role = await server.inject({ method: "PATCH", url: "/workspaces/tenant-1/members/member/role", headers: authHeaders(), payload: { role: "ADMIN" } });
...
test("pending invite counts against Starter seat limit and returns PLAN_LIMIT_EXCEEDED", async () => {
  ...
  const response = await server.inject({ method: "POST", url: "/workspaces/tenant-1/invitations", headers: authHeaders({ "x-user-id": "owner" }), payload: { email: "second@example.com", role: "MEMBER" } });
...
test("valid invitation accept activates member, audits acceptance, and prevents token reuse", async () => {
  ...
  const accepted = await server.inject({ method: "POST", url: "/invitations/fixed-token/accept", headers: { "x-user-email": "person@example.com", "x-correlation-id": "corr-2" } });
  const reused = await server.inject({ method: "POST", url: "/invitations/fixed-token/accept" });
