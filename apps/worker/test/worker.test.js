...
test('registers event ingestion, score recomputation, notification, claim lifecycle, render retry, publish, and scheduler workers on startup', async () => {
  ...
  assert.deepEqual(registrations.map((registration) => registration.queue.queueName), ['event.ingestion', 'crm.scoring', 'notification', 'marketplace.claim.lifecycle', 'marketplace.crm.conversion', 'marketplace.revenue.attribution', 'render.conversion.retry', 'marketplace.invite', 'marketplace.discovery', 'marketplace.qualification', 'publish', 'scheduler', 'marketplace.growth.loop']);
  assert.deepEqual(registrations.map((registration) => registration.worker.jobTypes[0]), ['event.ingestion', 'crm.score.recompute', 'notification.trial_reminder', 'marketplace.claim.reminder', 'marketplace.crm.conversion.execute', 'marketplace.revenue.attribution.evaluate', 'render.conversion.retry', 'marketplace.invite.send', 'marketplace.discovery.execute', 'marketplace.qualification.execute', 'publish.dispatch', 'scheduler.tick', 'marketplace.growth.loop.evaluate']);
  ...
  assert.equal(queues.isWorkerActive('revenue-attribution-worker'), true);
...
test('revenue attribution worker invokes runtime service and rejects tenant mismatched jobs', async () => {
  const calls = [];
  const runtime = createRuntimePorts();
  const app = createApp({
    events: { ingest: async () => ({ id: 'ingestion-1', tenantId: 'tenant-1' }) },
    revenueAttribution: {
      computeAttribution: async (context, input) => {
        calls.push({ context, input });
        return { status: 'ATTRIBUTED', dealId: input.dealId };
      },
    },
  }, runtime.ports);

  const result = await app.processJob({ job: createJob({
    jobId: 'revenue-attribution-1',
    queueName: 'marketplace.revenue.attribution',
    jobType: 'marketplace.revenue.attribution.evaluate',
    payload: { tenantId: 'tenant-1', dealId: 'deal-1' },
    idempotency: { tenantId: 'tenant-1', scope: 'JOB', key: 'tenant-1:revenue-attribution-1', replaySafe: true, conflictPolicy: 'SKIP_DUPLICATE' },
    scheduling: { tenantId: 'tenant-1', queueName: 'marketplace.revenue.attribution', priority: 'NORMAL' },
  }) });

  assert.equal(result.status, 'SUCCEEDED');
  assert.equal(calls.length, 1);
  assert.equal(calls[0].context.tenantId, 'tenant-1');
  assert.equal(calls[0].input.dealId, 'deal-1');

  const mismatched = await app.processJob({ job: createJob({
    jobId: 'revenue-attribution-2',
    queueName: 'marketplace.revenue.attribution',
    jobType: 'marketplace.revenue.attribution.evaluate',
    payload: { tenantId: 'tenant-2', dealId: 'deal-2' },
    idempotency: { tenantId: 'tenant-1', scope: 'JOB', key: 'tenant-1:revenue-attribution-2', replaySafe: true, conflictPolicy: 'SKIP_DUPLICATE' },
    scheduling: { tenantId: 'tenant-1', queueName: 'marketplace.revenue.attribution', priority: 'NORMAL' },
  }) });

  assert.equal(mismatched.status, 'DEAD_LETTERED');
  assert.equal(calls.length, 1);
});
