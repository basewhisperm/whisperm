...
import type { AuditLogRepository, DealsRepository, DraftInventoryRecord, DraftInventoryRepository, MarketplaceCaptureRecord, MarketplaceCaptureRepository, PipelineRepository } from "@whisperm/repositories";
import type { PersistenceCorrelationMetadata, TenantScoped } from "@whisperm/types";


const idSchema = z.string().min(1);
const acceptInputSchema = z.object({ claimantName: z.string().trim().min(1).max(120).optional(), acceptedTerms: z.literal(true) }).strict();
...
export interface SellerClaimPortalDependencies { readonly claimTokens: ClaimTokenRepository; readonly marketplaceCaptures: MarketplaceCaptureRepository; readonly draftInventories: DraftInventoryRepository; readonly pipelines: PipelineRepository; readonly deals: DealsRepository; readonly auditLogs: AuditLogRepository; readonly clock?: (() => Date) | undefined; }
...
export class SellerClaimPortalService {
  ...
  async accept(context: { readonly correlation: PersistenceCorrelationMetadata }, rawToken: string, input: unknown): Promise<{ readonly status: "CLAIMED"; readonly captureId: string; readonly draftInventoryId: string | null; readonly claimedAt: string }> {
    ...
    if (draft !== null && draft.status !== "CLAIMED") await this.deps.draftInventories.update(scope, draft.id, { status: "CLAIMED" });
    await this.audit({ tenantId: token.tenantId, correlation: context.correlation }, "MARKETPLACE_CLAIM_ACCEPTED", token.id, { marketplaceCaptureId: capture.id, draftInventoryId: draft?.id ?? null });
    return { status: "CLAIMED", captureId: capture.id, draftInventoryId: draft?.id ?? null, claimedAt };
  }
  ...
  private async moveStage(context: { readonly tenantId: string; readonly correlation: PersistenceCorrelationMetadata }, capture: MarketplaceCaptureRecord, stageName: StageName, tokenId: string): Promise<void> { if (capture.dealId == null || terminal.has(capture.status)) return; const currentStage = statusStage[capture.status] ?? "Captured"; if (currentStage === stageName) return; const pipeline = await this.deps.pipelines.findByDefaultKey(context.tenantId, "marketplace-acquisition"); const stage = pipeline?.stages.find((item) => item.name === stageName); if (stage === undefined) throw new SellerClaimPortalError({ code: "SERVICE_CONFLICT", message: `Marketplace Acquisition ${stageName} stage is missing`, status: 409, correlation: context.correlation }); await this.deps.deals.updateStage(context.tenantId, capture.dealId, stage.id); await this.deps.marketplaceCaptures.update({ tenantId: context.tenantId }, capture.id, { status: stageStatus[stageName] }); await this.audit(context, stageName === "Claim Started" ? "MARKETPLACE_CLAIM_STARTED" : "MARKETPLACE_CLAIM_STAGE_CLAIMED", tokenId, { marketplaceCaptureId: capture.id, previousStage: currentStage, currentStage: stageName }); }
