...
test('claimed acquisition without phone cannot convert seller or create conversion', async () => { const setup = deps({ capture: capture({ metadata: { sellerEmail: 'sam@example.com', sellerLocation: 'Austin', marketplaceSource: 'MARKET_TEST' } }), contact: contact({ phone: null }) }); await rejectsWithCode(() => new RenderSellerConversionService(setup.deps).convertClaimedSellerToRender(context, { tenantId: 'tenant-1', marketplaceCaptureId: 'capture-1' }), 'SERVICE_INVALID_STATE_TRANSITION'); assert.equal(setup.state.conversions.length, 0); assert.equal(setup.state.connectorCalls.length, 0); });
