...
const makeDeps = (overrides = {}) => {
  const state = { token: { ...baseToken, ...(overrides.token ?? {}) }, capture: { ...baseCapture, ...(overrides.capture ?? {}) }, draft: overrides.draft === null ? null : { ...baseDraft, ...(overrides.draft ?? {}) }, scheduled: [], notifications: [], audits: [] };
  const deps = {
...
test('sending invitation schedules Day 3, Day 6, and Day 7 lifecycle jobs', async () => {
  ...
  assert.equal(state.scheduled.length, 3);
});
