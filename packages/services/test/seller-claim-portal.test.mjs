...
function makeService(overrides = {}) {
  ...
  const service = new SellerClaimPortalService({
    ...
    pipelines: { async findByDefaultKey(tenantId, key) { assert.equal(tenantId, 'tenant-1'); assert.equal(key, 'marketplace-acquisition'); return { id: 'pipeline-1', tenantId, name: 'Marketplace Acquisition', defaultKey: key, stages: [{ id: 'stage-started', name: 'Claim Started' }, { id: 'stage-claimed', name: 'Claimed' }] }; } },
