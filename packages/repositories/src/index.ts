...
export interface PrismaPersistenceClient {
  ...
  readonly queueJob: PrismaDelegate;
...
export type UpdateDealInput = Partial<Pick<DealRecord, "value" | "currency" | "probability" | "closedAt" | "metadata">> & OptimisticLock;
...
export type UpdateSellerAcquisitionCampaignMemberInput = Partial<Pick<SellerAcquisitionCampaignMemberRecord, "contactId" | "dealId" | "status" | "metadata">>;
...
export interface MarketplaceCaptureRepository { create(context: TenantScoped, input: CreateMarketplaceCaptureInput): Promise<MarketplaceCaptureRecord>; findByListingUrl(context: TenantScoped, listingUrl: string): Promise<MarketplaceCaptureRecord | null>; findByExternalId(context: TenantScoped, externalId: string): Promise<MarketplaceCaptureRecord | null>; findById(context: TenantScoped, captureId: string): Promise<MarketplaceCaptureRecord | null>; findByDealId(context: TenantScoped, dealId: string): Promise<MarketplaceCaptureRecord | null>; findByIds(context: TenantScoped, captureIds: readonly string[]): Promise<readonly MarketplaceCaptureRecord[]>; list(context: TenantScoped, page?: PageRequest): Promise<Page<MarketplaceCaptureRecord>>; update(context: TenantScoped, captureId: string, input: UpdateMarketplaceCaptureInput): Promise<MarketplaceCaptureRecord>; }
...
export interface MarketplaceClaimTokenRepository { create(context: TenantScoped, input: CreateMarketplaceClaimTokenInput): Promise<MarketplaceClaimTokenRecord>; findById(context: TenantScoped, tokenId: string): Promise<MarketplaceClaimTokenRecord | null>; findByTokenHash(context: TenantScoped, tokenHash: string): Promise<MarketplaceClaimTokenRecord | null>; update(context: TenantScoped, tokenId: string, input: UpdateMarketplaceClaimTokenInput): Promise<MarketplaceClaimTokenRecord>; listClaimTokensByMarketplaceCaptureId(context: TenantScoped, marketplaceCaptureId: string): Promise<readonly MarketplaceClaimTokenRecord[]>; listClaimTokensByMarketplaceCaptureIds(context: TenantScoped, marketplaceCaptureIds: readonly string[]): Promise<readonly MarketplaceClaimTokenRecord[]>; }
...
export class PrismaDealsRepository implements DealsRepository {
  ...
  async update(workspaceId: string, dealId: string, input: UpdateDealInput): Promise<DealRecord> {
    const context = workspaceContext(workspaceId);
    ensureContext(context);
    return updateOptimistic(this.prisma.deal, dealRecordSchema, context, dealId, input);
  }
...
export class PrismaSellerAcquisitionCampaignRepository implements SellerAcquisitionCampaignRepository {
  ...
  async listMembersByCapture(context: TenantScoped, marketplaceCaptureId: string): Promise<readonly SellerAcquisitionCampaignMemberRecord[]> {
    ensureContext(context);
    const rows = await this.prisma.sellerAcquisitionCampaignMember.findMany({ where: withTenant(context, { marketplaceCaptureId, removedAt: null }) });
    return rows.map((row) => parseRecord(sellerAcquisitionCampaignMemberRecordSchema, row));
  }

  async updateMember(context: TenantScoped, memberId: string, input: UpdateSellerAcquisitionCampaignMemberInput): Promise<SellerAcquisitionCampaignMemberRecord> {
    ensureContext(context);
    const result = await this.prisma.sellerAcquisitionCampaignMember.updateMany({ where: withTenant(context, { id: memberId }), data: dataWithDefined(input) });
    if (result.count !== 1) notFound("Seller acquisition campaign member not found", { memberId });
    const row = await this.prisma.sellerAcquisitionCampaignMember.findFirst({ where: withTenant(context, { id: memberId }) });
    if (row === null) notFound("Seller acquisition campaign member not found", { memberId });
    return parseRecord(sellerAcquisitionCampaignMemberRecordSchema, row);
  }
}
...
export class PrismaMarketplaceCaptureRepository implements MarketplaceCaptureRepository {
  ...
  async findByIds(context: TenantScoped, captureIds: readonly string[]): Promise<readonly MarketplaceCaptureRecord[]> {
    ensureContext(context);
    const uniqueIds = [...new Set(captureIds.filter((id) => id.length > 0))];
    if (uniqueIds.length === 0) return [];
    const rows = await this.prisma.marketplaceCapture.findMany({ where: withTenant(context, { id: { in: uniqueIds } }), orderBy: { createdAt: "desc" } });
    return rows.map((row) => parseRecord(marketplaceCaptureRecordSchema, row));
  }

  async list(context: TenantScoped, page?: PageRequest): Promise<Page<MarketplaceCaptureRecord>> {
    ensureContext(context);
    const args = pageArgs(page);
    const rows = await this.prisma.marketplaceCapture.findMany({ where: cursorWhere(context, args.cursor), take: args.take, orderBy: { createdAt: "desc" } });
    return paginate(rows.map((row) => parseRecord(marketplaceCaptureRecordSchema, row)), args.take - 1);
  }
...
export class PrismaMarketplaceClaimTokenRepository implements MarketplaceClaimTokenRepository {
  ...
  async findById(context: TenantScoped, tokenId: string): Promise<MarketplaceClaimTokenRecord | null> {
    ensureContext(context);
